...
bool verbose=false;
if(verbose)
std::cout<<"Raman : Calling SetMax Step of StepMax " << std::endl;
                                                  ...
                                                  G4ForceCondition* condition )
{
  ...
  *condition = NotForced;
  ...
  if((fMaxChargedStep > 0.) &&
     (aTrack.GetVolume() != 0) &&
     aTrack.GetVolume()->GetName() == "PhysicalTiFoil"
    )
 //&&
...
bool verbose=false;
if(verbose)
  std::cout<<"Sehgal : StepLength from StepMax : " << ProposedStep << std::endl;
  return ProposedStep;
}
